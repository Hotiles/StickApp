...
| # | Change | Detail | Review ref |
|---|--------|--------|-----------|
| D1 ❌ *(dropped July 2026 after user research)* | **Multiple pattern files per project** | Asked, users didn't want it: the patterns they actually use are almost exclusively a single file with instructions and chart together, and more than one file per project is rare. The settled design is archived in `docs/design-d1-d7.md` should reality change. | §7, wish 7 |
| D2 ✅ *(shipped July 2026)* | **A second band** | Opt-in second band via the "Bandverktyg"-sheet ("Lägg till ett andra band") — a full band object (`viewState.band2`) reusing the overlay/drag/fit machinery wholesale, with its own colour, orientation, position and thickness. Each band carries its own `color` (distinct pastels so chart-row and instruction-line bands read apart) and its own `thicknessPt` override (D6's project-level `viewState.bandThickness` migrated onto the primary band at load; the field is dropped on next save so the two shapes can never disagree — no db migration, `normalizeBand` backfills). A compact active-band selector (colour dot + 1/2) in the toolbar picks which band the visibility/orientation/fit buttons target; both bands are always draggable directly. Single-band default is untouched. | §8, wish 8 |
| D3 ✅ *(shipped July 2026)* | **Tap-to-highlight size marker** | "Märk din storlek" in the Bandverktyg-sheet enters a marker mode: a tap drops a small ring in document coordinates (scales with zoom like the band, so it stays around the same number through the pattern); a tap on an existing ring erases it. Held the line — dots/rings only, no ink, text or shapes. Stored per page in `viewState.markers` ({ [page]: [{x, y}] }, 0–1 fractions), no schema migration (read as empty when absent, like D7's `lastMovedPage`); rides along in the project entity so backup/merge need no change. Swipe and double-tap-zoom are suppressed in marker mode so a tap never changes the page. | §8, wish 8 |
...
- Backup format must version-bump for D4; test restore-from-old-backup
...
- D1 Multiple files per project ❌ *(dropped after user research — users'
  patterns are almost exclusively single-file; design archived in
  `docs/design-d1-d7.md`)*
...
- D2 Second band + D3 size marker ✅ *(shipped July 2026 — shared band/annotation surface, built together as the plan intended)*
...
matter once people have project **history**. D4 stands alone now that D1
is dropped — an image "pattern" is a multi-page pattern entity in the
library (image page source behind the viewer's page abstraction), which
never needed D1's per-project file list. If user feedback screams earlier,
both can be pulled forward without dependency trouble.)*
